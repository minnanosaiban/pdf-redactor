...
$("clearAll").onclick = () => {
  ...
  history.length = 0;
  for (const k of Object.keys(dom)) drawOverlay(+k);
  updateCount();
};
...
function buildPages(w, h) {
  for (let p = 1; p <= numPages; p++) {
    ...
    dom[p] = { wrap, stage, view, overlay, rendered: false, rendering: false, preview: null };
    attachDraw(p);
  }
}
...
function drawOverlay(p) {
  ...
  for (const r of rectsOf(p)) draw(r, "rgba(220,0,0,0.35)", "rgba(220,0,0,0.9)");
...
function attachDraw(p) {
  ...
  o.addEventListener("pointerup", (e) => {
    ...
    start = null; moved = false; d.preview = null; drawOverlay(p); updateCount();
  });
}
